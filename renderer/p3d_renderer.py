...
class Pytorch3dRenderer(object):
    ...
    def render(self, verts, faces, bg_img, ft, vt, t):
        verts = verts.copy()
        faces = faces.copy()
        ...
        maps_tensor = torch.from_numpy(t).float().unsqueeze(0).cuda()
        faces_uvs_tensor = torch.from_numpy(ft.astype(np.int64)).long().unsqueeze(0).cuda()
        verts_uvs_tensor = torch.from_numpy(vt).float().unsqueeze(0).cuda()

        # set color
        # mesh_color = self.mesh_color.repeat(1, verts.shape[0], 1)
        textures = Textures(
            maps=maps_tensor,
            faces_uvs=faces_uvs_tensor,
